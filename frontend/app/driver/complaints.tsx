...
export default function DriverComplaints() {
  ...
  const fetchRides = async () => {
    ...
    try {
      ...
      const filtered = user ? allRides.filter((ride: any) => ride.driver_id === user.id) : [];
      setRides(filtered);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedRide.passenger_id) {
      Alert.alert('Erè', 'Kous sa a pa gen pasajè');
      return;
    }
    if (!message.trim()) {
    ...
    try {
      await complaintsAPI.create({
        target_user_type: 'passenger',
        target_user_id: selectedRide.passenger_id,
